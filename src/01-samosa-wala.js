...
export function createSamosaCart(ownerName, location) {
  ...
  return {
    ...
    sales: [],
    sellItem(itemName, quantity) {
      if (!itemName || typeof itemName !== "string") return -1;
      if (!quantity || typeof quantity !== "number" || quantity < 1) return -1;
    ...
    getDailySales() {
      ...
      return this.sales.reduce((sum, item) => sum + item.total, 0);
    },
    getPopularItem() {
      if (this.sales.length == 0) return null;
      return this.sales.sort((a, b) => b.quantity - a.quantity)[0].item;
    },
    moveTo(newLocation) {
